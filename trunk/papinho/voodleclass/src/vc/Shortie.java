...
public class Shortie extends HttpServlet {
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ( user!= null){		
			String login=String.format("I'm logged in as <strong>%s</strong>(<a href='%s'>sign out</a>)<br/>", request.getUserPrincipal().getName(),userService.createLogoutURL(thisURL));
			pw.println(login);
		}else {
		...
		pw.println("<textarea name='post' style='"+css+"'  cols='100' id='textarea' onfocus='set_bg(\"#f6F5F3\",this.id)' onblur='set_bg(\"#ffffff\",this.id)'>");
		pw.println("</textarea>");
		pw.println("<br/>");
